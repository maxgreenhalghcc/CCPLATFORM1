...
import { LiftIn, FadeIn, StaggerChildren, StaggerItem } from '@/app/components/motion';
  ...
  return (
    <AppShell>
      <div className="mx-auto w-full max-w-4xl px-6 py-16">
        ...
        <section className="mt-12 grid gap-8 rounded-2xl border border-border/[var(--border-alpha,0.5)] bg-card/80 p-10 shadow-card">
          <div>
            <h2 className="text-xl font-semibold tracking-tight">Ingredients</h2>
            <StaggerChildren className="mt-4 space-y-2 text-sm text-muted-foreground">
              {ingredients.length > 0 ? (
                ingredients.map((item) => (
                  <StaggerItem key={item}>
                    <div className="rounded-xl border border-border/40 bg-background/80 px-4 py-3 text-foreground">
                      {item}
                    </div>
                  </StaggerItem>
                ))
                ...
                <StaggerItem>
                  <div className="text-muted-foreground">Ingredients will be confirmed by the bar team.</div>
                </StaggerItem>
              )}
            </StaggerChildren>
          </div>
          ...
          <FadeIn delay={0.4}>
            <div className="grid gap-6 sm:grid-cols-2">
              <article className="rounded-2xl border border-border/[var(--border-alpha,0.5)] bg-card/80 shadow-card p-6">
